...
app.controller('uc1_ctrl', function($scope, $rootScope, $routeParams, $http) {
    ...
    $http({method: 'GET', url:  "./data/data-list.json"})
        ...
        function(response) {
            ...
            console.log($scope.tumorTypes.available);
            ...
            if($scope.tumorTypes.available.length>0) {
                ...
                $scope.loadTumorType($scope.tumorTypes.current, $scope.motifsType.current);
            }
    ...
    $scope.loadMotifType = function(motifsType) {
        $scope.loadTumorType($scope.tumorTypes.current, motifsType);
    }
